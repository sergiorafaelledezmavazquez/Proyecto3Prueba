...
//para filtrarlas y programar su JQUERY deacuerdo a su tipo
    $(document).ready(function(){
	  ...
	  $("a.jq_boton_calificar").click(function(event){	
		    var totalPreguntasTema=$("#tema1 fieldset").size();
		    var totalReactivosTema=$("#tema1 fieldset input.jq_correcta").size();
		    //0.0 obtengo elelemento al que se hizo clic 
            $element = $(this);
		   ...
		   //4.-imprimo el resultado
		    $("h3.jq_titulotema").empty().html(tema);
		    $("p.jq_numeroPreguntas").empty().html("PREGUNTAS: " + totalPreguntasTema);
		    $("p.jq_Reactivos").empty().html("REACTIVOS: " + totalReactivosTema); 
		    $("p.jq_ReactivosAcertados").empty().html("REACTIVOS ACERTADOS: " + resultado); 
		    var calificacion = (resultado * 100)/totalReactivosTema; 
		    $("p.jq_Calificacion").empty().html("CALIFICACION: " + calificacion); 	     
	  });			
});
